...
class Messages extends Component {
  ...
  unassignedMessagesClickedHandler = () => {
    this.setState((prevState) => ({
      showUnassignedMessages: !prevState.showUnassignedMessages,
    }));
  };
  ...
  render() {
    ...
    let page = (
      <Aux>
        <Button
          to="/unassigned-requests"
          as={Link}
          className="messages__button"
        >
          Unassigned Requests
        </Button>
        <Grid celled columns={12}>
          <Grid.Row stretched>
            <Grid.Column width={4}>
              <MessageList messages={messages} selectedId={current.id} />
            </Grid.Column>
    ...
    if (this.state.showUnassignedMessages) {
      page = <UnassignedMessages />;
    } else return <div className="messages__container">{page}</div>;
  }
}
